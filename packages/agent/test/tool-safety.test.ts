...
// ============================================================================
// truncateToolResult — overflow file
// ============================================================================

describe('truncateToolResult overflow', () => {
	let tempDir: string
	...
	function makeTempDir() {
		tempDir = mkdtempSync(
			join(tmpdir(), 'tool-overflow-test-')
		)
		return tempDir
	}

	test('writes full output to file when overflowDir is set', () => {
		...
		const filePath = join(dir, 'test-call-1.txt')
		const fileContent = readFileSync(filePath, 'utf-8')
		expect(fileContent).toBe(bigText)

		// Suffix should reference the file path
		const text =
			truncated.content[0].type === 'text'
				? truncated.content[0].text
				: ''
	...
	test('adds overflowPath to details', () => {
		const dir = makeTempDir()
		const result: AgentToolResult = {
			content: [
				{ type: 'text', text: 'x'.repeat(100_000) }
		...
		const details = truncated.details as Record<
			...
			unknown
		>
		expect(details.existing).toBe('value')
		expect(details.overflowPath).toBe(
			join(dir, 'test-call-2.txt')
		)
	})

	test('does not write file when overflowDir is not set', () => {
		const bigText = 'x'.repeat(100_000)
		const result = makeTextResult(bigText)

		const truncated = truncateToolResult(result, 10_000)

		// Should use default suffix without file path
		const text =
			truncated.content[0].type === 'text'
				? truncated.content[0].text
				: ''
	...
	test('falls back to Date.now() filename when no toolCallId', () => {
		const dir = makeTempDir()
		const result = makeTextResult('x'.repeat(100_000))
		...
		const details = truncated.details as Record<
			...
			unknown
		>
		const overflowPath = details.overflowPath as string
		expect(overflowPath).toMatch(/\/\d+\.txt$/)
	})

	test('no file written when result fits within limit', () => {
		const dir = makeTempDir()
		const result = makeTextResult('small text')

		const truncated = truncateToolResult(result, 50_000, {
			overflowDir: dir,
			toolCallId: 'should-not-exist'
		})

		expect(truncated).toEqual(result)
		expect(() =>
			readFileSync(
				join(dir, 'should-not-exist.txt'),
				'utf-8'
			)
		).toThrow()
	})
