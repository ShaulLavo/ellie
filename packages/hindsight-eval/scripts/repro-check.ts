...
for (let i = 0; i < Math.min(casesRun1.length, casesRun2.length); i++) {
	...
	if (r1.candidates.length !== r2.candidates.length) {
		console.error(
			`  MISMATCH [${r1.caseId}]: candidate count ${r1.candidates.length} vs ${r2.candidates.length}`
		)
		allMatch = false
	} else {
...
if (existsSync(baselineJsonPath)) {
	...
	if (casesRun1.length !== baseline.cases.length) {
		console.warn(
			`  WARNING: case count differs: ${casesRun1.length} (current) vs ${baseline.cases.length} (baseline)`
		)
		baselineMatch = false
	}
