...
export async function runPhase2Verification(
  ...
  const narrativeQuestions = generateTemporalNarrativeDataset(200, ingestEvents.length, DATASET_SEED)
...
export function compareRuns(
  ...
  const gateResults: GateResult[] = [
    ...candidate.gates.filter((g) => !g.gate.startsWith("Gate 6") && !g.gate.startsWith("Gate 7") && !g.gate.startsWith("Gate 8")),
    {
      gate: "Gate 6",
