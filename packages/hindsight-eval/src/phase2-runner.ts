 ...
 */
export async function runRollingIngest(
	events: RollingIngestEvent[]
): Promise<{
...
export async function runPhase2Verification(
	...
	const { outputDir, runId, gitSha, gateTestsPassed } =
		options
	...
	console.log(
		'[Phase 2] Generating rolling ingest dataset...'
	)
	const ingestEvents = generateRollingIngestDataset(
		800,
		DATASET_SEED
	)
	...
	// Save dataset
	writeFileSync(
		join(outputDir, 'rolling_ingest.v1.jsonl'),
		toJsonl(ingestEvents)
	)

	console.log(
		`[Phase 2] Running rolling ingest (${ingestEvents.length} events)...`
	)
	const { clusterCounts } =
		await runRollingIngest(ingestEvents)
	const drMetrics = computeDuplicateRatio(clusterCounts)

	console.log(
		`[Phase 2] Duplicate ratio: ${(drMetrics.duplicateRatio * 100).toFixed(2)}%`
	)
	...
	console.log(
		'[Phase 2] Generating temporal narrative dataset...'
	)
	...
	// Save dataset
	writeFileSync(
		join(outputDir, 'temporal_narrative.v1.jsonl'),
	...
	gates.push({
		...
		status: 'skip',
		description:
			'Narrative accuracy (baseline comparison needed)',
		details: {
...
export function compareRuns(
	...
	const baselineDR =
		baseline.metrics.duplicateRatio?.duplicateRatio ?? 0
	const candidateDR =
		candidate.metrics.duplicateRatio?.duplicateRatio ?? 0
	const baselineAcc =
		baseline.metrics.narrativeAccuracy?.accuracy ?? 0
	const candidateAcc =
		candidate.metrics.narrativeAccuracy?.accuracy ?? 0
	...
	const gateResults: GateResult[] = [
		...candidate.gates.filter(
			g =>
				!g.gate.startsWith('Gate 6') &&
				!g.gate.startsWith('Gate 7') &&
				!g.gate.startsWith('Gate 8')
		),
		{
			gate: 'Gate 6',
		...
		},
		{
			...
			status: 'skip',
			description:
				'Reproducibility (requires separate phase2-repro-check.ts run)',
			details: {}
		}
	]
	...
	const comparisonReport: Phase2ComparisonReport = {
		...
		improvements: {
			...
			duplicateRatioReductionPercent:
				gate6.reductionPercent,
			narrativeAccuracyImprovement: gate7.improvement,
			narrativeAccuracyImprovementPercent:
				gate7.improvementPercent
		},
