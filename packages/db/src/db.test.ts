  ...
  describe("dedupe", () => {
    ...
    it("returns existing event for duplicate dedupeKey", () => {
      const r1 = store.append({
        sessionId: "s1",
        type: "user_message",
        payload: { role: "user", content: [{ type: "text", text: "hello" }], timestamp: Date.now() },
        dedupeKey: "msg-1",
      })
      const r2 = store.append({
        sessionId: "s1",
        type: "user_message",
        payload: { role: "user", content: [{ type: "text", text: "different" }], timestamp: Date.now() },
        dedupeKey: "msg-1",
      })
    ...
    it("rejects invalid payload for user_message", () => {
      expect(() =>
        store.append({
          sessionId: "s1",
          type: "user_message",
          payload: { role: "invalid", content: "not-an-array" } as unknown as Record<string, unknown>,
        })
      ).toThrow()
    })
  ...
  describe("session isolation", () => {
    it("events are isolated between sessions", () => {
      ...
      store.append({
        sessionId: "a",
        type: "user_message",
        payload: { role: "user", content: [{ type: "text", text: "from a again" }], timestamp: 3000 },
      })
    ...
    it("appends 1,000 events", () => {
      ...
      for (let i = 0; i < 1_000; i++) {
        store.append({
          sessionId: "bulk",
          type: "user_message",
          payload: { role: "user", content: [{ type: "text", text: `msg ${i}` }], timestamp: Date.now() },
        })
      }
