// From https://github.com/evanw/thumbhash
...
export function rgbaToThumbHash(
	...
	for (let i = 0, j = 0; i < w * h; i++, j += 4) {
		...
		let g =
			avg_g * (1 - alpha) + (alpha / 255) * rgba[j + 1]
		let b =
			avg_b * (1 - alpha) + (alpha / 255) * rgba[j + 2]
		l[i] = (r + g + b) / 3
		p[i] = (r + g) / 2 - b
		q[i] = r - g
		a[i] = alpha
	...
	let [l_dc, l_ac, l_scale] = encodeChannel(
		l,
		max(3, lx),
		max(3, ly)
	)
	...
	let [a_dc, a_ac, a_scale] = hasAlpha
		? encodeChannel(a, 5, 5)
		: [0, [], 0]
...
export function thumbHashToRGBA(hash: Uint8Array): {
	...
	let lx = max(
		3,
		isLandscape ? (hasAlpha ? 5 : 7) : header16 & 7
	)
	let ly = max(
		3,
		isLandscape ? header16 & 7 : hasAlpha ? 5 : 7
	)
	...
	for (let y = 0, i = 0; y < h; y++) {
		for (let x = 0; x < w; x++, i += 4) {
			...
			for (
				let cx = 0, n = max(lx, hasAlpha ? 5 : 3);
				cx < n;
				cx++
			)
				fx[cx] = cos((PI / w) * (x + 0.5) * cx)
			for (
				let cy = 0, n = max(ly, hasAlpha ? 5 : 3);
				cy < n;
				cy++
			)
 ...
 */
export function thumbHashToApproximateAspectRatio(
	hash: Uint8Array
): number {
	let header = hash[3]
...
export function rgbaToDataURL(
	...
	let table = [
		0, 498536548, 997073096, 651767980, 1994146192,
		1802195444, 1303535960, 1342533948, -306674912,
		-267414716, -690576408, -882789492, -1687895376,
		-2032938284, -1609899400, -1111625188
	]
	let a = 1,
		b = 0
	for (
		let y = 0, i = 0, end = row - 1;
		y < h;
		y++, end += row - 1
	) {
		bytes.push(
