...
describe('extractTemporalRange', () => {
	it('returns undefined for non-temporal queries', () => {
		...
		expect(
			extractTemporalRange('Tell me about dogs', REFERENCE)
		).toBeUndefined()
		expect(
			extractTemporalRange('', REFERENCE)
		).toBeUndefined()
	})

	it("detects 'june 2024' → month range", () => {
		const range = extractTemporalRange(
			'june 2024',
			REFERENCE
		)
		expectRange(
	...
	it("detects 'last Saturday' → specific day", () => {
		const range = extractTemporalRange(
			'I received jewelry last Saturday',
			REFERENCE
		)
	...
	it("detects 'a couple of days ago' → 1-3 days ago range", () => {
		const range = extractTemporalRange(
			'a couple of days ago we discussed this',
			REFERENCE
		)
		expectRange(
			range,
	...
	it("detects 'a couple of weeks ago' → 1-3 weeks ago range", () => {
		const range = extractTemporalRange(
			'a couple of weeks ago we discussed this',
			REFERENCE
		)
		expectRange(
			range,
...
describe('temporal link behavior via retain pipeline', () => {
	...
	it('candidate within temporal window creates a link', async () => {
			...
			facts: [
				{
					...
					occurredStart: base - 2 * 60 * 60 * 1000
				}
			],
		...
		const second = await t.hs.retain(bankId, 'second', {
			facts: [
				{ content: 'Source memory', occurredStart: base }
			],
			eventDate: base,
			dedupThreshold: 0,
			consolidate: false
		...
		const links = getTemporalLinks(
			second.memories[0]!.id,
			first.memories[0]!.id
		)
	...
	it('candidate outside temporal window creates no link', async () => {
		...
		const first = await t.hs.retain(bankId, 'first', {
			facts: [
				{
					content: 'Old memory',
					occurredStart: base - 3 * 24 * 60 * 60 * 1000
				}
			],
		...
		const second = await t.hs.retain(bankId, 'second', {
			facts: [
				{ content: 'New memory', occurredStart: base }
			],
			eventDate: base,
			dedupThreshold: 0,
			consolidate: false
	...
	it('link weight decreases with temporal distance', async () => {
		...
		const close = await t.hs.retain(bankId, 'close', {
			facts: [
				{
					content: 'Close candidate',
					occurredStart: base - 1 * 60 * 60 * 1000
				}
			],
		...
		const far = await t.hs.retain(bankId, 'far', {
			facts: [
				{
					content: 'Far candidate',
					occurredStart: base - 18 * 60 * 60 * 1000
				}
			],
		...
		const toClose = getTemporalLinks(
			source.memories[0]!.id,
			close.memories[0]!.id
		)[0]
		const toFar = getTemporalLinks(
			source.memories[0]!.id,
			far.memories[0]!.id
		)[0]
	...
	it('minimum link weight is 0.3', async () => {
		...
		const edge = await t.hs.retain(bankId, 'edge', {
			facts: [
				{
					content: 'Edge candidate',
					occurredStart: base - 23 * 60 * 60 * 1000
				}
			],
...
describe('temporal fields written to DB', () => {
	...
	it('occurred_start, occurred_end, and mentioned_at are written after retain', async () => {
		...
		const pointStart = Date.parse(
			'2024-11-16T08:00:00.000Z'
		)
		...
		const retain = await t.hs.retain(
			bankId,
			'temporal fields',
			{
				eventDate,
				facts: [
					{
						content: 'Pottery workshop yesterday',
						occurredStart: pointStart,
						occurredEnd: pointEnd
					},
					{
						content: 'Alice visited Paris in February 2024',
						occurredStart: periodStart,
						occurredEnd: periodEnd
					}
				],
				dedupThreshold: 0,
				consolidate: false
	...
	it('mentioned_at is close to the event_date provided at retain time', async () => {
		...
		expect(
			Math.abs(mentionedAt! - eventDate)
		).toBeLessThanOrEqual(1000)
	})

	it('point event: occurred_start and occurred_end are within the same day', async () => {
		const occurredStart = Date.parse(
			'2024-11-16T08:00:00.000Z'
		)
		const occurredEnd = Date.parse(
			'2024-11-16T18:00:00.000Z'
		)
		...
		expect(memory.occurredEnd).not.toBeNull()
		const sameDay =
			new Date(memory.occurredStart!)
				.toISOString()
				.slice(0, 10) ===
			new Date(memory.occurredEnd!)
				.toISOString()
				.slice(0, 10)
	...
	it('temporal fields are present and populated in search/recall results', async () => {
		...
		await t.hs.retain(bankId, 'searchable temporal', {
			facts: [
				{
					content: 'Pottery workshop detail',
					occurredStart,
					occurredEnd
				}
			],
