...
describe('Gate 5: API Contract Verification', () => {
	...
	describe('listEpisodes', () => {
		it('returns episodes sorted by last_event_at DESC', async () => {
			...
			await t.hs.retain(bankId, 'first', {
				facts: [
					{
						content: 'Fact for session A xyz 111',
						factType: 'experience'
					}
				],
				session: 'session-a',
			...
			await t.hs.retain(bankId, 'second', {
				facts: [
					{
						content: 'Fact for session B xyz 222',
						factType: 'experience'
					}
				],
				session: 'session-b',
		...
		it('cursor pagination is deterministic', async () => {
			...
			for (let i = 0; i < 5; i++) {
				await t.hs.retain(bankId, `content-${i}`, {
					facts: [
						{
							content: `Fact ${i} unique xyz ${i * 111}`,
							factType: 'experience'
						}
					],
					session: `session-${i}`,
			...
			// Page 1
			const page1 = await t.hs.listEpisodes(bankId, {
				limit: 2
			})
		...
		it('second run with same cursor returns same results', async () => {
			for (let i = 0; i < 3; i++) {
				await t.hs.retain(bankId, `content-${i}`, {
					facts: [
						{
							content: `Fact ${i} unique abc ${i * 222}`,
							factType: 'experience'
						}
					],
					session: `session-${i}`,
			...
			expect(page2a.items[0]!.episodeId).toBe(
				page2b.items[0]!.episodeId
			)
		})
		...
		it('scope filters applied correctly — profile filter', async () => {
			...
			const aliceEpisodes = await t.hs.listEpisodes(
				bankId,
				{ profile: 'alice' }
			)
			...
			const bobEpisodes = await t.hs.listEpisodes(bankId, {
				profile: 'bob'
			})
		...
		it('scope filters applied correctly — project filter', async () => {
			await t.hs.retain(bankId, 'projA-fact', {
				facts: [
					{
						content: 'Project A fact xyz 111',
						factType: 'experience'
					}
				],
				project: 'project-a',
			...
			await t.hs.retain(bankId, 'projB-fact', {
				facts: [
					{
						content: 'Project B fact xyz 222',
						factType: 'experience'
					}
				],
				project: 'project-b',
			...
			const projA = await t.hs.listEpisodes(bankId, {
				project: 'project-a'
			})
			expect(projA.total).toBe(1)

			const projB = await t.hs.listEpisodes(bankId, {
				project: 'project-b'
			})
		...
		it('scope filters applied correctly — session filter', async () => {
			await t.hs.retain(bankId, 's1-fact', {
				facts: [
					{
						content: 'Session 1 fact xyz 111',
						factType: 'experience'
					}
				],
				session: 's1',
			...
			await t.hs.retain(bankId, 's2-fact', {
				facts: [
					{
						content: 'Session 2 fact xyz 222',
						factType: 'experience'
					}
				],
				session: 's2',
			...
			const s1 = await t.hs.listEpisodes(bankId, {
				session: 's1'
			})
		...
		it('limit clamps between 1 and 100', async () => {
			...
			const result2 = await t.hs.listEpisodes(bankId, {
				limit: 1000
			})
	...
	describe('narrative', () => {
		it('returns empty events for non-existent anchor', async () => {
			...
			expect(result.anchorMemoryId).toBe(
		...
		it('returns the anchor event in the result', async () => {
			...
			const anchorEvent = narr.events.find(
				e => e.memoryId === anchorId
			)
		...
		it('supports direction=before', async () => {
			await t.hs.retain(bankId, 'first', {
				facts: [
					{
						content: 'Earlier fact alpha xyz 111',
						factType: 'experience'
					}
				],
				consolidate: false
			...
			const r2 = await t.hs.retain(bankId, 'second', {
				facts: [
					{
						content: 'Later fact beta xyz 222',
						factType: 'experience'
					}
				],
				consolidate: false
		...
		it('supports direction=both (default)', async () => {
			await t.hs.retain(bankId, 'first', {
				facts: [
					{
						content: 'First fact alpha xyz 111',
						factType: 'experience'
					}
				],
				consolidate: false
			...
			const r2 = await t.hs.retain(bankId, 'second', {
				facts: [
					{
						content: 'Second fact beta xyz 222',
						factType: 'experience'
					}
				],
				consolidate: false
			...
			await t.hs.retain(bankId, 'third', {
				facts: [
					{
						content: 'Third fact gamma xyz 333',
						factType: 'experience'
					}
				],
				consolidate: false
		...
		it('returns events in deterministic order (event_time ASC)', async () => {
			await t.hs.retain(bankId, 'batch', {
				facts: [
					{
						content: 'Fact one alpha xyz 111',
						factType: 'experience'
					},
					{
						content: 'Fact two beta xyz 222',
						factType: 'experience'
					},
					{
						content: 'Fact three gamma xyz 333',
						factType: 'experience'
					}
				],
				consolidate: false
			...
			const result = await t.hs.retain(bankId, 'last', {
				facts: [
					{
						content: 'Fact four delta xyz 444',
						factType: 'experience'
					}
				],
				consolidate: false
			...
			for (let i = 0; i < narr.events.length - 1; i++) {
				expect(
					narr.events[i]!.eventTime
				).toBeLessThanOrEqual(narr.events[i + 1]!.eventTime)
			}
		})
		...
		it('traverses across episode boundaries via temporal links', async () => {
			...
			await t.hs.retain(bankId, 'ep1-fact', {
				facts: [
					{
						content: 'Episode 1 fact alpha xyz 111',
						factType: 'experience'
					}
				],
				consolidate: false
			...
			const r2 = await t.hs.retain(bankId, 'ep2-fact', {
				facts: [
					{
						content: 'new task Episode 2 fact beta xyz 222',
						factType: 'experience'
					}
				],
				consolidate: false
		...
		it('narrative events have correct NarrativeEvent shape', async () => {
			const result = await t.hs.retain(bankId, 'test', {
				facts: [
					{
						content: 'Shape test fact xyz 111',
						factType: 'experience'
					}
				],
				consolidate: false
		...
		it('steps parameter limits traversal depth', async () => {
			...
			for (let i = 0; i < 5; i++) {
				await t.hs.retain(bankId, `ep${i}-fact`, {
					facts: [
						{
							content: `Episode ${i} fact xyz ${i * 111}`,
							factType: 'experience'
						}
					],
					session: `session-${i}`,
