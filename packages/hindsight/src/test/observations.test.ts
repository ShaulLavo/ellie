...
describe('Entity extraction on retain', () => {
	...
	afterEach(() => {
		const pending = t.hs.listOperations(bankId, {
			status: 'pending'
		})
	...
	it('assigns entity types from pre-provided facts', async () => {
		...
		const google = result.entities.find(
			e => e.name === 'Google'
		)
...
describe('Entity mention ranking (TDD targets)', () => {
		...
		const entities = db.db
			...
			.where(
				(await import('drizzle-orm')).eq(
					db.schema.entities.bankId,
					bankId
				)
			)
			.all()

		const alice = entities.find(
			(e: { name: string; mentionCount: number }) =>
				e.name === 'Alice'
		)
		const bob = entities.find(
			(e: { name: string; mentionCount: number }) =>
				e.name === 'Bob'
		)
		expect(alice).toBeDefined()
		expect(bob).toBeDefined()
		expect(alice!.mentionCount).toBeGreaterThan(
			bob!.mentionCount
		)
	})
	...
	it('most frequently mentioned entity has highest mentionCount', async () => {
		await t.hs.retain(bankId, 'test', {
			facts: [
				{
					content: 'Alice studies math',
					entities: ['Alice']
				},
				{
					content: 'Alice studies physics',
					entities: ['Alice']
				},
	...
	it('entity mention counts are bank-scoped', async () => {
		...
		await t.hs.retain(bankId, 'test', {
			facts: [
				{ content: 'Alice in bank 1', entities: ['Alice'] }
			],
			consolidate: false
		})
		await t.hs.retain(bankId2, 'test', {
			facts: [
				{ content: 'Alice in bank 2', entities: ['Alice'] }
			],
			consolidate: false
		})
	...
	it('entities are ranked by mention count DESC', async () => {
		...
		for (let i = 1; i < entities.length; i++) {
			expect(
				entities[i - 1]!.mentionCount
			).toBeGreaterThanOrEqual(entities[i]!.mentionCount)
		}
	})
	...
	it('user entity is extracted when mentioned frequently', async () => {
		...
		for (let i = 0; i < 5; i++) {
			await t.hs.retain(bankId, 'test', {
				facts: [
					{
						content: `User Shaul performed action ${i}`,
						entities: ['Shaul']
					}
				],
				consolidate: false,
...
describe('Recall entity parameters (TDD targets)', () => {
	...
	it('recall accepts include_entities parameter', async () => {
		...
		expect(
			Array.isArray(result.memories[0]!.entities)
		).toBe(true)
	})
})
...
describe('Observation storage controls (TDD targets)', () => {
	...
	it('observations are not stored when disabled', async () => {
		...
		const observations = t.hs.listMemoryUnits(bankId, {
			factType: 'observation'
		})
		const experiences = t.hs.listMemoryUnits(bankId, {
			factType: 'experience'
		})
...
describe('Entity state tracking (TDD targets)', () => {
	...
	it('entity firstSeen is set on first mention', async () => {
		...
		await t.hs.retain(bankId, 'test', {
			facts: [
				{
					content: 'Alice started a project',
					entities: ['Alice']
				}
			],
			consolidate: false
	...
	it('entity lastUpdated is updated on subsequent mentions', async () => {
		...
		const first = db.db
			.select({
				lastUpdated: db.schema.entities.lastUpdated
			})
			.from(db.schema.entities)
			.where(
				and(
		...
		expect(second!.lastUpdated).toBeGreaterThanOrEqual(
			first!.lastUpdated
		)
	})
	...
	it('entity description can be updated', async () => {
		...
		const alice = entities.items.find(
...
describe('clearObservations (TDD targets)', () => {
	...
	it('clearObservations removes all observations from the bank', async () => {
		...
		const before = t.hs.listMemoryUnits(bankId, {
			factType: 'observation'
		})
		...
		const after = t.hs.listMemoryUnits(bankId, {
			factType: 'observation'
		})
	...
	it('clearObservations does not remove raw facts (experience/world)', async () => {
		await t.hs.retain(bankId, 'raw', {
			facts: [
				...
				{
					content: 'Coffee contains caffeine',
					factType: 'world',
					entities: ['Coffee']
				}
			],
		...
		expect(clearResult.deletedCount).toBeGreaterThanOrEqual(
			0
		)

		const rawAfter = t.hs.listMemoryUnits(bankId)
		const rawAfterSet = new Set(
			rawAfter.items.map(item => item.id)
		)
		for (const id of rawIds) {
			expect(rawAfterSet.has(id)).toBe(true)
		}
		expect(rawAfter.total).toBeGreaterThanOrEqual(
			rawIds.length
		)
	})
})
...
describe('Operation lifecycle (TDD targets)', () => {
	...
	async function waitForOperation(
		...
		ReturnType<TestHindsight['hs']['getOperationStatus']>
	> {
		const started = Date.now()
		for (;;) {
			const status = t.hs.getOperationStatus(
				bankId,
				operationId
			)
			if (
	...
	it('completed operation is removed from the pending operations list', async () => {
		...
		const finalStatus = await waitForOperation(
			submitted.operationId
		)
	...
	it('failed operation has errorMessage set', async () => {
		const submitted =
			await t.hs.submitAsyncRefreshMentalModel(
				bankId,
		...
		expect(
			(finalStatus.errorMessage ?? '').length
		).toBeGreaterThan(0)
	})
})
...
describe('Core parity: test_observations.py', () => {
	...
	it('entity extraction on retain', async () => {
		...
		expect(
			t.hs.listMemoryUnits(bankId).items.length
		).toBeGreaterThan(0)
	})
	...
	it('search with include entities', async () => {
		...
		const recall = await t.hs.recall(bankId, 'Bob', {
