...
describe("WorkingMemoryStore", () => {
  ...
  test("returns positive boost after touch", () => {
    ...
    expect(boost).toBeCloseTo(0.20, 2) // 0.20 * exp(0)
  })
...
describe("scoreCognitive (with DB)", () => {
  ...
  test("ranks by probe when no access history", () => {
    const candidates: CognitiveCandidate[] = [
      { id: "a", semanticSimilarity: 0.3, accessCount: 0, lastAccessed: null, encodingStrength: 1.0 },
      { id: "b", semanticSimilarity: 0.9, accessCount: 0, lastAccessed: null, encodingStrength: 1.0 },
  ...
  test("access history boosts score via base activation", () => {
    const now = Date.now()
    const candidates: CognitiveCandidate[] = [
      { id: "fresh", semanticSimilarity: 0.5, accessCount: 10, lastAccessed: now - 1000, encodingStrength: 1.0 },
  ...
  test("deterministic tie-break by id ASC", () => {
    const candidates: CognitiveCandidate[] = [
      { id: "bbb", semanticSimilarity: 0.5, accessCount: 0, lastAccessed: null, encodingStrength: 1.0 },
  ...
  test("all scores are deterministic across runs", () => {
    ...
    const candidates: CognitiveCandidate[] = [
      { id: "a", semanticSimilarity: 0.7, accessCount: 5, lastAccessed: now - 86400000, encodingStrength: 1.0 },
