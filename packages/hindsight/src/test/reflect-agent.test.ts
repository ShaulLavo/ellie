...
describe("Reflect agent with mocked LLM", () => {
  ...
  it("handles 'functions.done' prefix in tool call", async () => {
    const result = await createLoop([
      ...
      [{ id: "2", name: "functions.done", arguments: { answer: "Test answer", memory_ids: ["mem-1"] } }],
    ])
  ...
  it("handles 'call=functions.done' prefix", async () => {
    const result = await createLoop([
      [{ id: "1", name: "recall", arguments: { query: "test" } }],
      [{ id: "2", name: "call=functions.done", arguments: { answer: "Test answer", memory_ids: ["mem-1"] } }],
    ])
    expect(result.text).toBe("Test answer")
  })
  ...
  it("recovers from unknown tool call", async () => {
    const result = await createLoop([
      ...
      [{ id: "3", name: "done", arguments: { answer: "Recovered successfully", memory_ids: ["mem-1"] } }],
    ])
  ...
  it("recovers from tool execution error", async () => {
    ...
    const result = await createLoop([
      ...
      [{ id: "3", name: "done", arguments: { answer: "Recovered from error", memory_ids: ["mem-1"] } }],
    ], tools)
