...
describeWithLLM('Ollama dry run (qwen2.5:7b-instruct)', () => {
	...
	it('extracts facts from transcript turns', async () => {
		const result = await t.hs.retain(
			...
			[
				...
				{
					role: 'assistant',
					content: 'I really enjoy TypeScript! The type system helps catch bugs early.'
				},
	...
	it('full pipeline: retain → recall → reflect', async () => {
		...
		// Step 3: Reflect
		const reflectResult = await t.hs.reflect(bankId, 'Summarize what you know about David.', {
			budget: 'low'
		})
