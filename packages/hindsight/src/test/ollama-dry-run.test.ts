...
describeWithLLM(
	...
	() => {
		...
		it('recalls seeded facts by query', async () => {
			...
			for (const m of result.memories) {
				console.log(
					`  - [${m.score.toFixed(3)}] ${m.memory.content}`
				)
			}
		...
		it('handles empty bank gracefully', async () => {
			...
			console.log(
				'[reflect empty] answer length:',
				result.answer.length
			)
			console.log(
				'[reflect empty] answer:',
				result.answer.slice(0, 300)
			)
		...
		it('consolidation completes without throwing', async () => {
			...
			console.log(
				'[consolidate] observationsCreated:',
				result.observationsCreated
			)
			console.log(
				'[consolidate] observationsUpdated:',
				result.observationsUpdated
			)
			console.log(
				'[consolidate] observationsMerged:',
				result.observationsMerged
			)
			console.log('[consolidate] skipped:', result.skipped)
			console.log(
				'[consolidate] memoriesProcessed:',
		...
		it('full pipeline: retain → recall → reflect', async () => {
			...
			console.log(
				'[pipeline] reflect answer:',
				reflectResult.answer.slice(0, 300)
			)
