...
describe('recall cognitive mode', () => {
	...
	test("mode='hybrid' returns same structure as default", async () => {
		const defaultResult = await t.hs.recall(bankId, 'hiking')
		const hybridResult = await t.hs.recall(bankId, 'hiking', { mode: 'hybrid' })
		expect(hybridResult.query).toBe('hiking')
		expect(hybridResult.memories.length).toBe(defaultResult.memories.length)
	...
	test("mode='cognitive' returns scored memories", async () => {
		const result = await t.hs.recall(bankId, 'hiking', { mode: 'cognitive' })
	...
	test("mode='cognitive' scores are sorted descending", async () => {
		const result = await t.hs.recall(bankId, 'programming', { mode: 'cognitive' })
...
describe('access write-through on recall', () => {
	...
	test('recall increments access_count for returned memories', async () => {
		// Check initial state
		const hdb = (t.hs as unknown as { hdb: HindsightDatabase }).hdb
		const before = hdb.db.select().from(hdb.schema.memoryUnits).all()
		...
		for (const row of after) {
			if (returnedIds.includes(row.id)) {
				expect(row.accessCount).toBe((initialCounts.get(row.id) ?? 0) + 1)
			}
		}
	})
	...
	test('recall updates last_accessed for returned memories', async () => {
		...
		for (const row of rows) {
			if (returnedIds.has(row.id)) {
				expect(row.lastAccessed).toBeGreaterThanOrEqual(before)
	...
	test('encoding_strength is capped at 3.0', async () => {
		...
		hdb.sqlite.run(`UPDATE hs_memory_units SET encoding_strength = 2.99 WHERE bank_id = ?`, [
			bankId
		])
	...
	test('write-through works in both hybrid and cognitive modes', async () => {
		...
		const afterHybrid = hdb.db.select().from(hdb.schema.memoryUnits).all()
		...
		for (const row of afterCognitive) {
			if (returnedInCognitive.has(row.id)) {
				...
				expect(row.accessCount).toBeGreaterThan(hybridCounts.get(row.id) ?? 0)
			}
		}
	})
})
