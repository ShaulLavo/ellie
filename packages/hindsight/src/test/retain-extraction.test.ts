...
describeWithLLM(
	'Fact extraction quality (real LLM)',
	() => {
		...
		async function extract(
			...
			const result = await t.hs.retain(bankId, content, {
				consolidate: false,
				eventDate:
					options.eventDate ?? CANONICAL_EVENT_DATE,
...
Marcus felt anxious about the upcoming interview.`,
				...
				const found = emotionalIndicators.filter(term =>
					text.includes(term)
				)
			...
			it('preserves mixed emotions (excitement and anxiety)', async () => {
				...
				expect(memories.length).toBeGreaterThan(0)
				expect(
					['excited', 'anxious', 'excited', 'anxiety'].some(
...
The music was so loud I could barely hear myself think.`,
				...
				const found = sensoryIndicators.filter(term =>
					text.includes(term)
				)
		...
		describe('relative to absolute date conversion', () => {
			...
			it("converts 'last Saturday' to absolute date", async () => {
				const memories = await extract(
					'Last Saturday I met Emily for lunch.'
				)
				expect(memories.length).toBeGreaterThan(0)
				const text = lowerJoined(memories)
				expect(
					text.includes('saturday') ||
						memories.some(m => m.occurredStart != null)
				).toBe(true)
			}, 60_000)

			it("converts 'two weeks ago' to approximate date", async () => {
				const memories = await extract(
					'Two weeks ago I submitted the application.'
				)
				expect(memories.length).toBeGreaterThan(0)
				expect(
					memories.some(
						memory => memory.occurredStart != null
					)
				).toBe(true)
			}, 60_000)
		...
		describe('agent vs world classification', () => {
			it("classifies 'I went hiking' as experience or world", async () => {
				const memories = await extract(
					'I went hiking in the mountains yesterday with my friend Tom. We reached the summit by noon.'
				)
				expect(memories.length).toBeGreaterThan(0)
				expect(
					memories.some(
		...
		describe('speaker attribution', () => {
			it('attributes facts to the correct speaker in a conversation', async () => {
				const transcript: RetainContentInput = [
					{
						role: 'Marcus',
						content: 'I predict the Rams win 27-24.'
					},
					{
						role: 'Jamie',
						content: 'I predict the Niners win 27-13.'
					}
				]
				const memories = await extract(transcript, {
					context:
						'Podcast predictions between Marcus and Jamie'
				})
				const text = lowerJoined(memories)
				expect(memories.length).toBeGreaterThan(0)
				expect(
					text.includes('rams') || text.includes('niners')
				).toBe(true)
			}, 60_000)
			...
			it("resolves 'I' to the speaker's name when available", async () => {
				const transcript: RetainContentInput = [
					{
						role: 'Alice',
						content: 'I moved to Berlin last year.'
					}
				]
				const memories = await extract(transcript, {
				...
				expect(memories.length).toBeGreaterThan(0)
				expect(
					text.includes('alice') || !text.includes(' i ')
				).toBe(true)
			}, 60_000)
			...
			it('handles multi-speaker conversations', async () => {
				const transcript: RetainContentInput = [
					...
					{
						role: 'Ravi',
						content: 'I invested as an angel.'
					},
					{
						role: 'Dana',
						content: 'We launched in October.'
					}
				]
				const memories = await extract(transcript, {
				...
				const text = lowerJoined(memories)
				expect(
					['startup', 'invested', 'launched'].some(term =>
						text.includes(term)
					)
				).toBe(true)
			}, 60_000)
		...
		describe('irrelevant content filtering', () => {
			it("filters out 'How are you?' greetings", async () => {
				...
				const text = lowerJoined(memories)
				expect(
					text.includes('madrid') || text.includes('move')
				).toBe(true)
			}, 60_000)
		...
		describe('output ratio', () => {
			it('output/input token ratio stays below 5x', async () => {
				...
				const outputChars = memories.reduce(
			...
			it('output/input token ratio stays below 6x for large inputs', async () => {
				const input =
					'Last weekend I visited my friend in San Francisco. '.repeat(
						60
					) +
					'We discussed AI healthcare projects and my plan to switch teams next quarter.'
				const memories = await extract(input, {
					context: 'Long journal entry'
				})
				const outputChars = memories.reduce(
					(sum, memory) => sum + memory.content.length,
					0
