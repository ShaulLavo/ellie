...
describe('Core parity: test_document_tracking.py', () => {
	...
	it('document creation and retrieval', async () => {
		await t.hs.retain(bankId, 'doc', {
			facts: [{ content: 'document-linked memory' }],
			documentId: 'doc-a',
			consolidate: false
		})
	...
	it('document upsert', async () => {
		...
		await t.hs.retain(bankId, 'doc2', {
			facts: [{ content: 'document-linked memory v2' }],
			documentId: 'doc-a',
	...
	it('memory without document', async () => {
		const result = await t.hs.retain(bankId, 'plain', {
			facts: [{ content: 'memory without document' }],
			consolidate: false
		})
