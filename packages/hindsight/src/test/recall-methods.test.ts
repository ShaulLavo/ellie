	...
	describe('graph retrieval', () => {
		...
		it('applies entity frequency filtering', async () => {
			const retained = await t.hs.retain(bankId, 'test', {
				facts: [
					{
						content: 'Alice writes Python services',
						factType: 'world',
						entities: ['Alice', 'Python']
					},
					{
						content: 'Bob writes Python pipelines',
						factType: 'world',
						entities: ['Bob', 'Python']
					},
					{
						content: 'Carol writes Python scripts',
						factType: 'world',
						entities: ['Carol', 'Python']
					}
				],
				consolidate: false
			})
				...
				facts: [
					{
						...
						occurredStart: now - 3_600_000,
						occurredEnd: now
					},
					{
						content: 'Conference was last month',
						occurredStart: now - 30 * 86_400_000,
						occurredEnd: now - 25 * 86_400_000
					}
				],
				consolidate: false
			})
		...
		it('returns empty when no memories in range', async () => {
			...
			await t.hs.retain(bankId, 'test', {
				facts: [
					{
						content: 'Old event',
						occurredStart: now - 365 * 86_400_000,
