...
describe("Core parity: test_fact_ordering.py", () => {
  ...
  it("fact ordering within conversation", async () => {
    ...
    const result = await t.hs.retain(bankId, "retain", { facts: [{ content: "Alice visited Rome", factType: "experience", confidence: 0.95, entities: ["Alice", "Rome"], tags: ["travel"], validFrom: eventDate }], eventDate, context: "travel diary", metadata: { source: "unit-test" }, documentId: "doc-test_fact_ordering_within_conversation", consolidate: false })
  ...
  it("multiple documents ordering", async () => {
    const eventDate = Date.now() - 86_400_000
    const result = await t.hs.retain(bankId, "retain", { facts: [{ content: "Alice visited Rome", factType: "experience", confidence: 0.95, entities: ["Alice", "Rome"], tags: ["travel"], validFrom: eventDate }], eventDate, context: "travel diary", metadata: { source: "unit-test" }, documentId: "doc-test_multiple_documents_ordering", consolidate: false })
    expect(result.memories.length).toBe(1)
    expect(result.memories[0]!.content.length).toBeGreaterThan(0)
  })
