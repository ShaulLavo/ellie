...
const LEAKED_JSON_SUFFIX =
	/\s*```(?:json)?\s*\{[^}]*(?:"(?:observation_ids|memory_ids|mental_model_ids)"|\})\s*```\s*$/is
const LEAKED_JSON_OBJECT =
	/\s*\{[^{]*"(?:observation_ids|memory_ids|mental_model_ids|answer)"[^}]*\}\s*$/is
const TRAILING_IDS_PATTERN =
	/\s*(?:observation_ids|memory_ids|mental_model_ids)\s*[=:]\s*\[.*?\]\s*$/is
