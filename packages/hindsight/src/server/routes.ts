  ...
  return {
    ...
    locationRecord: async (raw: unknown, params) => {
      ...
      if (
        ...
        context.memoryId.length === 0
      ) {
        throw new Error("Missing 'context.memoryId' field");
      }
      const allowedActivity = new Set(["access", "retain", "recall"]);
      if (
        context.activityType != null &&
        (typeof context.activityType !== "string" ||
          !allowedActivity.has(context.activityType))
      ) {
        throw new Error("Invalid 'context.activityType' field");
      }
      hs.locationRecord(
        params.bankId,
        input.path,
        {
          ...
          activityType: context.activityType as
            | "access"
            | "retain"
            | "recall"
            | undefined,
...
function createHindsightApp(hs: Hindsight) {
  ...
  return (
    new Elysia()
      ...
      .patch(
        "/banks/:bankId",
        ({ body, params }) => invoke("updateBank", body, params),
        {
          params: bankParamsSchema,
      ...
      .delete(
        "/banks/:bankId",
        ({ params }) => invoke("deleteBank", undefined, params),
        {
          params: bankParamsSchema,
        },
      )
      ...
      .get(
        "/banks/:bankId/stats",
        ({ params }) => invoke("getBankStats", undefined, params),
        {
          params: bankParamsSchema,
        },
      )
      ...
      .post(
        ...
        ({ body, params }) => invoke("narrative", body, params),
        {
          params: bankParamsSchema,
