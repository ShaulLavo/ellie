...
describe("Hindsight HTTP Server Integration", () => {
	describe("Bank CRUD", () => {
		...
		it("lists all banks via GET /banks", async () => {
			...
			const banks = await rpc.listBanks({ input: {} }) as any[]
		...
		it("gets a bank by ID via GET /banks/:bankId", async () => {
			...
			const found = await rpc.getBank({ bankId: created.id, input: {} }) as any
		...
		it("returns error for non-existent bank", async () => {
			await expect(
				rpc.getBank({ bankId: "nonexistent-id-12345", input: {} }),
			).rejects.toThrow()
		})
		...
		it("deletes a bank via DELETE /banks/:bankId", async () => {
			...
			await rpc.deleteBank({ bankId: bank.id, input: {} })
			// After delete, getting it should throw (404)
			await expect(
				rpc.getBank({ bankId: bank.id, input: {} }),
			).rejects.toThrow()
		})
