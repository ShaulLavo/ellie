...
export async function recall(
	...
	const methods = options.methods ?? ['semantic', 'fulltext', 'graph', 'temporal']
	...
	const semanticTask = methodSet.has('semantic')
		? timedAsync(() => searchSemantic(hdb, memoryVec, bankId, query, candidateLimit))
	...
	if (mode === 'cognitive') {
		...
		rankedCandidates = fused
			.map((candidate, index) => {
				...
				const combinedScore =
					0.6 * crossEncoderScoreNormalized + 0.2 * rrfNormalized + 0.1 * temporal + 0.1 * recency
	...
	const effectiveScope = options.scope ? resolveScope(options.scope) : undefined
	...
	for (const candidate of rankedCandidates) {
		...
		if (options.minConfidence != null && row.confidence < options.minConfidence) {
			continue
		}
		if (options.factTypes && !options.factTypes.includes(row.factType as FactType)) {
			continue
		}
...
function computeRecency(
	...
	if (occurredStart != null && occurredEnd != null) {
		...
		anchor = occurredStart ?? occurredEnd ?? row.mentionedAt ?? row.eventDate ?? row.createdAt
	}
 ...
 */
function updateAccessMetadata(hdb: HindsightDatabase, memoryIds: string[], now: number): void {
