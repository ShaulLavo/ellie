...
export async function recall(
	...
	const timeRange =
		options.timeRange ?? extractTemporalRange(query)
		...
		? await timedAsync(() =>
				searchGraph(
					...
					candidateLimit,
					{
						...
						temporalSeedMemoryIds: temporalTimed.hits.map(
							hit => hit.id
						)
					}
				)
	...
	if (methodSet.has('semantic'))
		resultSets.push(semanticTimed.hits)
	if (methodSet.has('fulltext'))
		resultSets.push(fulltextTimed.hits)
	if (methodSet.has('graph'))
		resultSets.push(graphTimed.hits)
	if (methodSet.has('temporal'))
		resultSets.push(temporalTimed.hits)
	...
	if (mode === 'cognitive') {
		...
		const cogCandidates: CognitiveCandidate[] = fused
			.map(candidate => {
				...
				return {
					id: candidate.id,
					semanticSimilarity:
						semanticScoreById.get(candidate.id) ?? 0,
		...
		const cogScored = scoreCognitive(
			hdb,
			cogCandidates,
			now
		)
		...
		rankedCandidates = cogScored.map((scored, index) => {
			...
			return {
				...
				rank: index + 1,
				sources:
					candidate.sources as RankedCandidate['sources'],
		...
		const rrfMax = Math.max(
			...fused.map(candidate => candidate.score)
		)
		const rrfRange = rrfMax - rrfMin

		let crossEncoderRawScores = fused.map(
			candidate => candidate.score
		)
		let crossEncoderNormalizedScores = fused.map(
			candidate =>
		...
		const temporalById = new Map(
			temporalTimed.hits.map(hit => [hit.id, hit.score])
		)
		rankedCandidates = fused
			.map((candidate, index) => {
				...
				const rrfNormalized = normalizeRrf(
					candidate.score,
					rrfMin,
					rrfRange
				)
				const temporal =
					temporalById.get(candidate.id) ?? 0.5
				const recency = computeRecency(row, now)
				const crossEncoderScoreNormalized =
					crossEncoderNormalizedScores[index] ?? 0.5
				const combinedScore =
				...
				return {
					...
					sources:
						candidate.sources as RankedCandidate['sources'],
					rrfScore: candidate.score,
					rawCrossEncoderScore:
						crossEncoderRawScores[index] ?? 0,
			...
			.filter(
				(candidate): candidate is RankedCandidate =>
					candidate != null
			)
	...
	if (locationSignals.length > 0) {
		const scopeFilter = options.scope
			...
			: undefined
		const signalPathMap = resolveSignalsToPaths(
			hdb,
			bankId,
			locationSignals,
			scopeFilter
	...
	const scopeMode: ScopeMode =
		(options as RecallOptionsWithScope).scopeMode ??
		'strict'
	...
	const junctionsByMemoryId = new Map<
		string,
		typeof allJunctions
	>()
	...
	const allEntityIds = [
		...new Set(allJunctions.map(j => j.entityId))
	]
	const allEntityRows =
		allEntityIds.length > 0
			? hdb.db
					...
					.where(
						inArray(hdb.schema.entities.id, allEntityIds)
					)
					.all()
			: []
	const entityById = new Map(
		allEntityRows.map(e => [e.id, e])
	)
	...
	for (const candidate of rankedCandidates) {
		...
		const junctions =
			junctionsByMemoryId.get(candidate.id) ?? []
		...
		if (options.entities && options.entities.length > 0) {
			const entityNames = new Set(
				entityRows.map(entityRow =>
					entityRow!.name.toLowerCase()
				)
			)
			const hasMatch = options.entities.some(name =>
	...
	if (
		...
		const packCandidates: PackCandidate[] = memories.map(
			m => ({
				id: m.memory.id,
		...
		phaseMetrics.push({
			...
			details: {
				...
				gistCount: packResult.packed.filter(
					p => p.mode === 'gist'
				).length,
				fullCount: packResult.packed.filter(
					p => p.mode === 'full'
				).length
			}
		})
	...
	const trace: RecallTrace | undefined = options.enableTrace
		? {
				...
				selectedMemoryIds: memories.map(
...
function buildMethodTrace(
	methods: Array<
		[RecallTraceMethodResult['methodName'], TimedHits]
	>
): RecallTraceMethodResult[] {
	return methods
		.filter(
			([, timed]) =>
				timed.durationMs > 0 || timed.hits.length > 0
		)
...
function buildEntityPayload(
	...
	for (const [
		entityId,
		entityState
	] of entityStates.entries()) {
		const memoryIds = Array.from(entityState.memoryIds)
		const tokenCost =
			estimateTokens(entityState.name) + memoryIds.length
		if (
			maxEntityTokens != null &&
			usedTokens + tokenCost > maxEntityTokens
		) {
			break
		}
...
function buildChunkPayload(
	...
	for (const scored of memories) {
		...
		if (memoryIdByChunkId.has(scored.memory.chunkId))
			continue
		memoryIdByChunkId.set(
			scored.memory.chunkId,
			scored.memory.id
		)
	...
	const chunkRows =
		chunkIdsOrdered.length > 0
			? hdb.db
					...
					.where(
						inArray(hdb.schema.chunks.id, chunkIdsOrdered)
					)
					.all()
			: []
	...
	for (const scored of memories) {
		if (
			scored.memory.chunkId &&
			payload[scored.memory.chunkId]
		)
			continue
		const chunkId = `memory:${scored.memory.id}`
		if (payload[chunkId]) continue
		const fallback =
			scored.memory.sourceText ?? scored.memory.content
