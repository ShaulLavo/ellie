 ...
 *
 * This is the single source of truth for all Hindsight data shapes.
 * TypeScript types are inferred from schemas via `v.InferOutput`.
 *
 * Imported by `@ellie/router` to define the appRouter, and by
 * `@ellie/hindsight` itself to derive types for internal use.
 *
 * IMPORTANT: This file must NOT import from `@ellie/router` or
 * `@ellie/rpc` — it only depends on `valibot` to avoid circular deps.
 */
