...
export interface DurableStreamOptions extends StreamHandleOptions {
  ...
  /**
   * Optional pluggable transport. When provided, all network
   * operations delegate to this transport instead of using raw fetch.
   *
   * Use this to route operations through Elysia Treaty or other
   * RPC layers while keeping the DurableStream API surface identical.
   */
...
export class DurableStream {
  ...
  async head(opts?: { signal?: AbortSignal }): Promise<HeadResult> {
    ...
    if (this.#transport) {
      response = await this.#transport.head({
  ...
  async delete(opts?: { signal?: AbortSignal }): Promise<void> {
    ...
    if (this.#transport) {
      response = await this.#transport.delete({
        signal: opts?.signal ?? this.#options.signal,
      })
    } else {
      const { requestHeaders, fetchUrl } = await this.#buildRequest()
  ...
  async close(opts?: CloseOptions): Promise<CloseResult> {
    ...
    if (this.#transport) {
      ...
      response = await this.#transport.close({
        body,
        headers,
        signal: opts?.signal ?? this.#options.signal,
      })
  ...
  async #appendDirect(
    ...
    if (this.#transport) {
      ...
      if (opts?.seq) {
        headers[STREAM_SEQ_HEADER] = opts.seq
      }

      response = await this.#transport.append({
        body: encodedBody,
        headers,
        signal: opts?.signal ?? this.#options.signal,
      })
  ...
  async stream<TJson = unknown>(
    ...
    if (this.#transport) {
      ...
      const isJsonHint = options?.json === true
      ...
      const firstResponse = result.firstResponse

      // Extract metadata from first response headers
      const contentType =
        firstResponse.headers.get(`content-type`) ?? undefined
      const initialOffset =
        firstResponse.headers.get(STREAM_OFFSET_HEADER) ?? startOffset
      const initialCursor =
        firstResponse.headers.get(STREAM_CURSOR_HEADER) ?? undefined
      const initialUpToDate = firstResponse.headers.has(
        STREAM_UP_TO_DATE_HEADER
      )
      const initialStreamClosed =
        firstResponse.headers.get(STREAM_CLOSED_HEADER)?.toLowerCase() ===
        `true`
      const isJsonMode =
        isJsonHint || (contentType?.includes(`application/json`) ?? false)
      ...
      return new StreamResponseImpl<TJson>({
        ...
        firstResponse,
        abortController,
        fetchNext: result.fetchNext,
        startSSE: result.startSSE,
        encoding,
      })
    }
