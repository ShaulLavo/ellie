...
describe(`StreamManager`, () => {
  ...
  describe(`call`, () => {
    let procServer: ReturnType<typeof Bun.serve>
    let procBaseUrl: string

    beforeAll(() => {
      procServer = Bun.serve({
        port: 0,
        async fetch(req) {
          const url = new URL(req.url)

          // POST /banks/:bankId/recall
          const recallMatch = url.pathname.match(/^\/banks\/([^/]+)\/recall$/)
          if (recallMatch && req.method === `POST`) {
            const body = await req.json()
            return Response.json({
              bankId: recallMatch[1],
              query: body.query,
              results: [],
            })
          }
          ...
          // GET /banks
          if (url.pathname === `/banks` && req.method === `GET`) {
            return Response.json([{ id: `b1`, name: `Bank 1` }])
          }

          // DELETE /banks/:bankId
          const deleteMatch = url.pathname.match(/^\/banks\/([^/]+)$/)
      ...
      procBaseUrl = `http://localhost:${procServer.port}`
    })

    afterAll(() => {
      procServer.stop(true)
    })
    ...
    it(`sends POST with JSON body`, async () => {
      ...
      expect(result).toEqual({ bankId: `b1`, query: `hiking`, results: [] })
    })
    ...
    it(`sends GET with query params`, async () => {
      ...
      expect(result).toEqual([{ id: `b1`, name: `Bank 1` }])
    })
