...
// Scalar subquery
const avgPrice = db
	.select({ value: avg(products.price) })
	.from(products)
...
const topAuthors = db.$with('top_authors').as(
	db
		.select({
			id: authors.id,
			name: authors.name,
			postCount: sql<number>`COUNT(${posts.id})`.as(
				'post_count'
			)
		})
		.from(authors)
...
const recentPosts = db.$with('recent_posts').as(
	db
		...
		.from(posts)
		.where(
			gt(posts.createdAt, sql`NOW() - INTERVAL '30 days'`)
		)
)
...
const result = await db
	...
	.leftJoin(
		recentPosts,
		eq(activeUsers.id, recentPosts.authorId)
	)
```
