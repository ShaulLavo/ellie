...
export const users = pgTable(
	...
	table => ({
		...
		cityStatusIdx: index('city_status_idx').on(
			table.city,
			table.status
		)
	})
...
const activeUsersInNYC = await db
	...
	.from(users)
	.where(
		and(eq(users.city, 'NYC'), eq(users.status, 'active'))
	)
```
...
// Define schema
export const userStats = pgMaterializedView(
	'user_stats'
).as(qb =>
	qb
		.select({
...
class MetricsLogger {
	...
	getStats() {
		return Array.from(this.queries.entries()).map(
			([query, stats]) => ({
				query,
				count: stats.count,
