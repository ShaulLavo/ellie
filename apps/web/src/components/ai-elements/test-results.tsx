...
const TestResultsContext =
	createContext<TestResultsContextType>({})
...
const TestSuiteContext =
	createContext<TestSuiteContextType>({
		name: '',
		status: 'passed'
	})

export type TestSuiteProps = ComponentProps<
	typeof Collapsible
> & {
	name: string
	status: TestStatus
...
const statusIcons: Record<TestStatus, React.ReactNode> = {
	...
	running: (
		<CircleDashedIcon className="size-4 animate-pulse" />
	),
