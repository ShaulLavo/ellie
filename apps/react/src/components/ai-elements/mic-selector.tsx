...
const MicSelectorContext =
	createContext<MicSelectorContextType>({
		data: [],
...
export type MicSelectorProps = ComponentProps<
	typeof Popover
> & {
	defaultValue?: string
	value?: string | undefined
...
export const MicSelector = ({
	...
	const { devices, loading, hasPermission, loadDevices } =
		useAudioDevices()
...
export type MicSelectorTriggerProps = ComponentProps<
	typeof Button
>
	...
	useEffect(() => {
		...
		const resizeObserver = new ResizeObserver(entries => {
			for (const entry of entries) {
				const newWidth = (entry.target as HTMLElement)
					.offsetWidth
				if (newWidth) {
					setWidth?.(newWidth)
				}
...
export type MicSelectorContentProps = ComponentProps<
	typeof Command
> & {
...
export const MicSelectorContent = ({
	...
	const { width, onValueChange, value } = useContext(
		MicSelectorContext
	)
...
export type MicSelectorInputProps = ComponentProps<
	typeof CommandInput
> & {
	value?: string
	defaultValue?: string
...
export const MicSelectorList = ({
	...
	return (
		<CommandList {...props}>{children(data)}</CommandList>
	)
}

export type MicSelectorEmptyProps = ComponentProps<
	typeof CommandEmpty
>
...
export type MicSelectorItemProps = ComponentProps<
	typeof CommandItem
>
...
export const MicSelectorItem = (
	...
	const { onValueChange, onOpenChange } = useContext(
		MicSelectorContext
	)
