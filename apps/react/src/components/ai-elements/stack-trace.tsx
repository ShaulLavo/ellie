...
const StackTraceContext =
	createContext<StackTraceContextValue | null>(null)
...
const parseStackFrame = (line: string): StackFrame => {
	...
	if (withParensMatch) {
		const [, functionName, filePath, lineNum, colNum] =
			withParensMatch
		const isInternal =
	...
	const withoutFnMatch = trimmed.match(
...
export type StackTraceCopyButtonProps = ComponentProps<
	typeof Button
> & {
	onCopy?: () => void
