...
function getSelectedMessageIds(
	...
	const intersectsContainer = ranges.some(range =>
		rangeIntersectsNode(range, container)
	)
	...
	for (const node of messageNodes) {
		...
		const intersectsAnyRange = ranges.some(range =>
...
export async function writeMarkdownToSystemClipboard(
	markdown: string
) {
	if (
	...
	if (
		...
		const textPlain = new Blob([plainText], {
			type: 'text/plain'
		})
		const textMarkdown = new Blob([markdown], {
			type: 'text/markdown'
		})
